...
# CHANGE based on what new users need to be processed
NEEDS_PROCESSING = ['M059', 'M060']
...
def add_timestamp():
    for dname in os.listdir(DIR):
        ...
        if dname in NEEDS_PROCESSING:
            ...
            def add_date(series):
                series['datetime'] = str(timestamp + dt.timedelta(seconds=int(series.iloc[0])))
                return series
            for fname in os.listdir(DIR+dname):
                if fname.startswith('.') or fname.startswith('unisens.') or '_' in fname:
                    continue
                print(fname)
                if (os.stat(DIR+dname+'/'+fname).st_size == 0):
                    print ('empty file')
                    continue
