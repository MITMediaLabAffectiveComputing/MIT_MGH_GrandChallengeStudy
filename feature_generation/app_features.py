...
def calculate_daily_call(intervals):
    ...
    for dname in os.listdir(DIR):
        ...
        def extract_values(series):
            ...
            series['type'] = MyConstants.get_app_type(series['info'])
            return series
        ...
        for app_type in MyConstants.APP_TYPES:
...
df_daily = calculate_daily_call(MyConstants.intervals)
...
df_daily.to_csv(MyConstants.feature_dir+'daily_appUsage.csv', index=False)
