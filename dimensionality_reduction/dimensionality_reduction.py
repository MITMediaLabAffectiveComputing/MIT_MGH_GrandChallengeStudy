...
def preprocess_survey_x_y():
    HAMD = pd.read_csv(MyConstants.data_dir+'daily_survey_HAMD_individualized.csv')
    HAMD = HAMD.rename(columns={'eveningGuitly': 'eveningGuilty'})
    # change the line below to df=HAMD for using both MDD and HC
    df = HAMD[HAMD['group']=='MDD'].reset_index(drop=True) #only MDD
    df = df.reset_index(drop=True)

    # drop timestamp information and labels
    all_df = df.drop(['Name','group',
    ...
    all_df = MyConstants.convert_one_hot_str(all_df, col='ID')
    y_cols = ['ID'] + ['individualized_' + col for col in MyConstants.LABEL_FACTORS] + ['original_' + col for col in
                                                                                         MyConstants.LABEL_FACTORS] + [
                 'baseline_' + col for col in MyConstants.LABEL_FACTORS]
    x_df = all_df.drop(y_cols+['date'], inplace=False, axis=1)
    y_df = all_df[y_cols]
...
def reduce_dimensionality(df, max_n, threshold):
    ...
    return x_df_standardized, x_df, optimal_n